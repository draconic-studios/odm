...
Destination met for v1: multi-platform GitHub Release **v0.1.1** ships four host triples + `SHA256SUMS`; `scripts/install.sh` curl|sh installs to `~/.local/bin` with checksum + `odm --version`; docs/README/website treat curl as primary and source as secondary. Children: [[issues-166-release-ci-policy]], [[issues-167-multi-target-release-workflow]], [[issues-168-install-sh]], [[issues-169-install-docs-website]], [[issues-170-first-cut-v0-1-1]] (all closed after human-gated tag/publish). Windows/musl/signing remain out of scope.
...
### 2026-08-02 close-out

All five children closed. Release https://github.com/hembrow-innovations/odm/releases/tag/v0.1.1 verified; install smoke OK.
