...
- [x] `for_kv_map` / `single_match` warnings gone from `doctor_worktree.rs`
- [x] Doctor orphan/dirty tests still pass
- [x] `cargo test` green
- [x] No intentional behavior change to check ids or fixable flags
...
Clippy-only cleanup in `worktree_dirty_checks`: iterate `ws.config.projects.keys()` (`for_kv_map`); replace `match` on `is_clean` with `if let Ok(false) = ...` (`single_match`). Orphan loop unchanged. Ids/messages/`fixable: false` and soft-skip on clean/err preserved. `cargo test` green; `cargo clippy --all-targets -p odm-core -- -D warnings` clean.
